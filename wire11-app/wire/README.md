...
## What changed in this pass: mobile, paste formatting, copy, bug fixes

This pass focused on making the app work well on phones (iOS and Android)
and on tablets, on preserving formatting when people paste multi-line
text, and on a handful of real bugs:

- **The composer is now a multi-line, auto-growing textarea** instead of
  a single-line input. Pasting something with line breaks or indentation
  (a list, a code snippet, a quoted email) keeps that formatting exactly —
  both while typing it and in how it's displayed to everyone else in the
  room. Enter sends the message; Shift+Enter adds a line break, and IME
  composition (for CJK input methods) is handled correctly so it doesn't
  send mid-composition.
- **Copy button on every message.** Hover (or tap, on touch) the small
  icon on a message to copy its exact text to the clipboard — useful for
  grabbing a multi-line message, a code block, or anything pasted with
  specific formatting.
- **Fixed a real cross-device bug:** `crypto.randomUUID()` only works in
  "secure contexts" (HTTPS, or `localhost`). Testing over a plain
  `http://<your-lan-ip>:3000` link — exactly what this README suggests
  for trying the app on a second phone — silently broke room creation on
  every device that wasn't `localhost`. There's now a manual fallback ID
  generator for that case, so LAN testing on real phones actually works.
- **Mobile/touch polish:** inputs are sized to avoid the iOS
  auto-zoom-on-focus bug, tap targets are bigger, the ugly gray tap
  highlight on Android/iOS is removed, pull-to-refresh no longer
  triggers accidentally while scrolling the message list
  (`overscroll-behavior`), and safe-area insets are respected on
  notched phones (in both portrait and landscape).
- **Installable on Android/iOS home screens** via a new
  `public/manifest.json`, so "Add to Home Screen" gives it a proper name,
  icon, and standalone (no browser chrome) window.
- **A dedicated send icon on narrow screens** so the composer doesn't feel
  cramped on small phones.

## What changed in an earlier pass

That pass focused on turning a working prototype into something you'd
...
public/manifest.json Web app manifest (Android/iOS "Add to Home Screen")
